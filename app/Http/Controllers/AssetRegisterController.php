...
class AssetRegisterController extends Controller {
	...
	public function getTree(){
		$data = AssetRegister::where('active','=',1)
	...
	public function postAddsubnode(){
		...
		$asset->parent = Input::get('node_id');
	...
	public function getFormsub8(){
		...
		$assets = DB::table('asset_registers')
            ->join('ref_categories', 'asset_registers.cat_id', '=', 'ref_categories.id')
            ->join('ref_parts', 'asset_registers.part_id', '=', 'ref_parts.id')
            ->join('ref_types', 'asset_registers.type_id', '=', 'ref_types.id')
            ->where('asset_registers.active','=',1)
            ->select('asset_registers.*','ref_categories.description as cat_desc','ref_parts.description as part_desc','ref_types.description as type_desc')
            ->get();

		return View::make('asset_register.form_sub8')
					->with('assets',$assets)
	...
	public function getFormlevel(){
		...
		if (Input::get('level')==3) {
			...
			foreach (Complex::where('active','=',1)
				->get() as $com) {
				$complexs[$com->id] = $com->name;
			}


			return View::make('asset_register.level_three')
				->with(compact('complexs'))
				->with(compact('assets'))
				->with(compact('complex_detail'))
				->with('status',$status)
				->with('node',Input::get('id'));
			...
			$asset_basic_failure = DB::Select(
				DB::raw("SELECT * FROM asset_basic_failure
			...
			$equip = DB::Select(DB::raw("
				...
				on e.part_id = c.id
				where e.active = 1 AND e.category_id = ".$parantdata->cat_id." 
				AND e.type_id = ".$parantdata->type_id." AND 
			...
			$unders = array();
			$unders = DB::table('asset_registers')
				->select('*')
				->where('id','>',Input::get('id'))
				->where('level','>',Input::get('level'))
            	->where('active','=',1)
            	->where('project_id','=',Session::get('project_id'))
            	->orderBy('parent', 'asc')->get();
			return View::make('asset_register.level_seven')
				->with('unders',$unders)
				->with(compact('assets'));
		}else{
			return View::make('asset_register.form_other')
				->with('unders',$unders)
				->with(compact('assets'));
		}
	}
	...
	public function getComplexform(){
		...
		$header_consequence = DB::table('complex_detail_default')
				->select('complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows','complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_id'))
            	->whereIn('complex_detail_default.type', array(6))
            	->get();

        $rows_consequence = DB::table('complex_detail_default')
				->select('complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows','complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_id'))
            	->whereIn('complex_detail_default.type', array(4))
            	->get();


		$consequence = DB::table('asset_complex_detail')
				->select('asset_complex_detail.ref1','asset_complex_detail.id','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(1))
            	->get();

        $occorrence = DB::table('asset_complex_detail')
				->select('asset_complex_detail.ref1','asset_complex_detail.id','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(2))
            	->get();

        $detection = DB::table('asset_complex_detail')
				->select('asset_complex_detail.ref1','asset_complex_detail.id','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(3))
            	->get();

        if (empty($consequence)) {
        	$consequence = DB::table('complex_detail_default')
				->select('complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows','complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_id'))
            	->whereIn('complex_detail_default.type', array(1))
            	->get();
        }

        if (empty($occorrence)) {
        	$occorrence = DB::table('complex_detail_default')
				->select('complex_detail_default.ref1','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows','complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_id'))
            	->whereIn('complex_detail_default.type', array(2))
            	->get();
        }

        if (empty($detection)) {
        	$detection = DB::table('complex_detail_default')
				->select('complex_detail_default.ref1','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows','complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_id'))
            	->whereIn('complex_detail_default.type', array(3))
            	->get();
        }

        if (Input::get('risk')==1) {
        	//กรณีเรียกจาก Level 8
        	return View::make('asset_register.view_risk_detail_complex')
				->with('consequence',$consequence)
				->with('header_consequence',$header_consequence)
				->with('rows_consequence',$rows_consequence)
				->with('header_consequence',$header_consequence)
				->with('rows_consequence',$rows_consequence)
				->with('header_consequence',$header_consequence)
				->with('rows_consequence',$rows_consequence)
				->with('occorrence',$occorrence)
				->with('detection',$detection);
        }else{
        	return View::make('asset_register.detail_complex')
				->with('consequence',$consequence)
				->with('header_consequence',$header_consequence)
				->with('rows_consequence',$rows_consequence)
				->with('header_consequence',$header_consequence)
	...
	public function getColorform(){
		...
		$color_header = DB::table('asset_complex_detail')
				->select('asset_complex_detail.id','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(7))
            	->get();

        $color_rows = DB::table('asset_complex_detail')
				->select('asset_complex_detail.id','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(4))
            	->get();

		$complex_color = DB::table('asset_complex_detail')
				->select('asset_complex_detail.id','asset_complex_detail.ref1','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(5))
            	->get();

		if (empty($complex_color)) {
				$color_header = DB::table('complex_detail_default')
				->select('complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows',
            		'complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_id'))
            	->whereIn('complex_detail_default.type', array(7))
            	->get();

            	$color_rows = DB::table('complex_detail_default')
				->select('complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows',
            		'complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	...
            	$complex_color = DB::table('complex_detail_default')
				->select('complex_detail_default.id','complex_detail_default.ref1','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows',
            		'complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_id'))
            	->whereIn('complex_detail_default.type', array(5))
            	->get();

		}

		return View::make('asset_register.table_color_default')
				->with('complex_color',$complex_color)
				->with('color_header',$color_header)
				->with('color_rows',$color_rows);
	}
	...
	public function postUpdatecolor(){
		...
		$complex_color = DB::table('asset_complex_detail')
				->select('asset_complex_detail.id','asset_complex_detail.complex_id','asset_complex_detail.ref1',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('com_id'))
            	->where('asset_complex_detail.node','=',Input::get('nd_id'))
            	->where('asset_complex_detail.id','=',Input::get('color_id'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->where('asset_complex_detail.type','=', 5)
            	->get();


        if (!empty($complex_color)) {

        	$rs = DB::table('asset_complex_detail')
            ->where('id', Input::get('color_id'))
            ->where('node',Input::get('nd_id'))
            ->where('complex_id',Input::get('com_id'))
            ->update(['description' => $color_code,'ref1'=>Input::get('ref1'),'updated_by'=> Auth::user()->id]);
            ...
            if ($rs==1) {
           	...
           	$comp_default = array();
        	$comp_default = DB::table('complex_detail_default')
				->select('complex_detail_default.type','complex_detail_default.ref1','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows',
            		'complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('com_id'))
            	->where('complex_detail_default.type' ,'=', 5)
            	->get();
           	foreach ($comp_default as $cold) {
           		DB::table('asset_complex_detail')->insert(
    				[
    					'node' => Input::get('nd_id'),
    					'complex_id' => Input::get('com_id'),
    					'rows' => $cold->rows,
			...
			$rs = DB::table('asset_complex_detail')
            	->where('ref_id', Input::get('color_id'))
            	->where('node',Input::get('nd_id'))
            	->where('complex_id',Input::get('com_id'))
            	->update(['description' => $color_code,'ref1' => Input::get('ref1'),'updated_by'=> Auth::user()->id]);
	...
	public function postSaveconseq(){
		...
		$consequence = DB::table('asset_complex_detail')
				->select('asset_complex_detail.type','asset_complex_detail.ref1','asset_complex_detail.id','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_conseq'))
            	->where('asset_complex_detail.node','=',Input::get('node_conseq'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(1))
            	->get();
        ...
        if (empty($consequence)) {
        	echo "insert";
        	$consequence = DB::table('complex_detail_default')
				->select('complex_detail_default.ref1','complex_detail_default.type','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows','complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_conseq'))
            	->whereIn('complex_detail_default.type', array(1))
            	->get();
            ...
            foreach ($consequence as $conseq) {
            	...
            	if (!empty($val)) {
            		DB::table('asset_complex_detail')->insert(
    					[
    						'node' => Input::get('node_conseq'),
    						'complex_id' => Input::get('complex_conseq'),
    						'rows' => $conseq->rows,
    						'columns' => $conseq->columns,
    						'description' => $val,
    						'type' => $conseq->type,
    						'ref1' => $conseq->ref1,
    						'project_id'=> Session::get('project_id'),
	...
	public function postSaveocc(){
		...
		$occorrence = DB::table('asset_complex_detail')
				->select('asset_complex_detail.type','asset_complex_detail.ref1','asset_complex_detail.id','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_occ'))
            	->where('asset_complex_detail.node','=',Input::get('node_occ'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(2))
            	->get();
        ...
        if (empty($occorrence)) {
        	...
        	$occorrence = DB::table('complex_detail_default')
				->select('complex_detail_default.ref1','complex_detail_default.type','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows','complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_occ'))
            	->whereIn('complex_detail_default.type', array(2))
            	->get();
            ...
            foreach ($occorrence as $occ) {
            	...
            	DB::table('asset_complex_detail')->insert(
    				[
    					'node' => Input::get('node_occ'),
    					'complex_id' => Input::get('complex_occ'),
    					'rows' => $occ->rows,
    					'columns' => $occ->columns,
    					'description' => $val,
    					'type' => $occ->type,
	...
	public function postSavedec(){
		...
		$detection = DB::table('asset_complex_detail')
				->select('asset_complex_detail.type','asset_complex_detail.ref1','asset_complex_detail.id','asset_complex_detail.complex_id',
            		'asset_complex_detail.columns','asset_complex_detail.rows','asset_complex_detail.description')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('complex_dec'))
            	->where('asset_complex_detail.node','=',Input::get('node_dec'))
            	->where('asset_complex_detail.project_id','=',Session::get('project_id'))
            	->whereIn('asset_complex_detail.type', array(3))
            	->get();
        ...
        if (empty($detection)) {
        	...
        	$detection = DB::table('complex_detail_default')
				->select('complex_detail_default.ref1','complex_detail_default.type','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows','complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('complex_dec'))
            	->whereIn('complex_detail_default.type', array(3))
            	->get();
            foreach ($detection as $dec) {
            	...
            	DB::table('asset_complex_detail')->insert(
    				[
    					'node' => Input::get('node_dec'),
    					'complex_id' => Input::get('complex_dec'),
    					'rows' => $dec->rows,
    					'columns' => $dec->columns,
    					'description' => $val,
    					'type' => $dec->type,
	...
	public function getType(){
		...
		$types = DB::Select(DB::raw("SELECT be.id,be.category_id,c.description,be.type_id,be.part_id
				...
				ON be.type_id = c.id
				WHERE be.active = 1
				AND be.project_id = ".Session::get('project_id')." AND be.category_id = ".Input::get('cat_id')
		));
		return View::make('asset_register.selected.type')
				->with('types',$types);
	}
	...
	public function getPart(){
		...
		return View::make('asset_register.selected.part')
				->with('parts',$parts);
	}

	public function getFuncfailure(){
		$func_failure = DB::Select(DB::raw(
"select f.id,bf.cause,bf.mode,f.worst_case from asset_basic_failure f
left join(
...
where f.active = 1 and f.project_id = ".Session::get('project_id')." and f.node = ".Input::get('node')
		...
		return View::make('asset_register.table.func_failure')
			->with('func_failure',$func_failure);
	}
	...
	public function getBasicfailureform(){
		...
		return View::make('asset_register.form.func_failure')
			->with(compact('failuremode'));
	}
	...
	public function getCause(){
		...
		return View::make('asset_register.selected.cause')
				->with('failurecause',$failurecause);
	}
	...
	public function postSavefuncfailure(){
		...
		if (empty($failurecause)) {
			$rs = DB::table('asset_basic_failure')->insert(
    				[
    					'node' => Input::get('node'),
    					'basic_failure_id' => Input::get('basic_failure_id'),
    					'worst_case'=>Input::get('worst_case'),
    					'project_id'=> Session::get('project_id'),
    					'created_by'=> Auth::user()->id,
    				]
			);
	...
	public function getDeletefuncfailure(){
		$rs = DB::table('asset_basic_failure')->where('id', Input::get('id'))
            	->update(
            	[
    				'active' => 0,
    				'project_id'=> Session::get('project_id'),
...
and f.project_id = ".Session::get('project_id')));
		...
		return View::make('asset_register.form.edit_func_failure')
				->with('assets_func',$assets_func)
				->with('mode',$mode);
	...
	public function getPicturelevel(){
		...
		return View::make('asset_register.images.picture')
	...
	public function getColourcal(){
		...
		if ((!empty($asset->severity)&&(!empty($asset->occur))&&($asset->detect))) {
			...
			return View::make('asset_register.table.colour_eight')
					->with('step2',$step2);
		}
	}
	...
	public function getServoccdec(){
		...
		return View::make('asset_register.table.servrity_occ_dec')
				->with('assets',$assets);
	...
	public function postUpdateservoccdec(){
		$rs = DB::table('asset_registers')->where('id', Input::get('node_id'))
            	->update(
            	[
            		'severity' => Input::get('serv_id'),
					'occur' => Input::get('occ_id'),
					'detect' => Input::get('detection_id'),
    				'project_id'=> Session::get('project_id'),
    				'updated_by'=> Auth::user()->id,
    			]
            );
...
and rs.id = ".Input::get('node_id')))[0];
		...
		return View::make('asset_register.table.basic_equip')
				->with('assets',$assets);
	...
	public function postSaveformlevel(){
			...
			$check_color = array();
			$check_color = $comp_color = DB::table('asset_complex_detail')
				->select('*')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('com_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
        ...
        if (empty($check_color)) {
        	//กรณี้ Save แบบ Default
           	$comp_color = DB::table('complex_detail_default')
				->select('complex_detail_default.type','complex_detail_default.ref1','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows',
            		'complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
	...
	public function getTaskselection(){
		...
		$func_failure = DB::Select(DB::raw(
"select f.id,bf.cause,bf.mode,f.worst_case,f.basic_failure_id from asset_basic_failure f
left join(
	select s.id,c.description as 'cause',m.description as 'mode'
	from basic_failures s
	left join ref_failure_cause c
...
on f.basic_failure_id = bf.id
where f.active = 1 and f.project_id = ".Session::get('project_id')." and f.node = ".Input::get('node_id')
		));
		...
		$order_type = array();
		$order = DB::Select(DB::raw("select * from ref_order_type where active = 1"));
		foreach ($order as $od) {
				$order_type[$od->order_type_id] = $od->name;
		}
	...
	public function getBasictask(){
		...
		return View::make('asset_register.selected.task_type')
				->with(compact('types'));
	...
	public function getBasictaskfinal(){
		...
		return View::make('asset_register.selected.task_list')
				->with(compact('type_list'));
	}
	...
	public function postSavetaskselection(){
		...
		if (!empty(Input::get('task_selection_id'))) {
			$rs = DB::table('task_selection')
            			->where('task_selection_id',Input::get('task_selection_id'))
            			->update(
            	[
    			'asset_basic_failure_id' => Input::get('asset_basic_failure_id'),
    			'node'=> Input::get('node_id'),
    			'failure_effect_id' => Input::get('failure_effect_id'),
    			'evident_id' => Input::get('evident_id'),
    			'interval_num' => Input::get('interval_num'),
    			'interval' => Input::get('interval'),
    			'basic_task_id' => Input::get('basic_task_id'),
    			'activity_status_id' => Input::get('activity_status_id'),
    			'activity_detail' => Input::get('activity_detail'),
			...
			$rs = DB::table('task_selection')->insert(
    		[
    			'asset_basic_failure_id' => Input::get('asset_basic_failure_id'),
    			'node'=> Input::get('node_id'),
    			'failure_effect_id' => Input::get('failure_effect_id'),
...
on tk.order_type_id=ot.order_type_id
left join ref_activity_status a
on tk.activity_status_id = a.activity_status_id
where tk.active = 1 and tk.project_id = ".Session::get('project_id')." and tk.node = ".Input::get('node_id')));

		return View::make('asset_register.table.table_task_selection')
			->with('taskselection',$taskselection);
	...
	public function postSetdetailcomplex(){
			...
			$check_columns_color = DB::table('asset_complex_detail')
				->select('*')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('com_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.type','=',7)
            	->get();
            if (empty($check_columns_color)) {
            	$comp_columns_color = array();
            	$comp_columns_color = DB::table('complex_detail_default')
				->select('complex_detail_default.type','complex_detail_default.ref1','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows',
            		'complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('com_id'))
            	->where('complex_detail_default.type','=',7)
            	->get();
            ...
            $check_columns_conq = DB::table('asset_complex_detail')
				->select('*')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('com_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.type','=',6)
            	->get();

            if (empty($check_columns_conq)) {
            	$comp_columns_conq = array();
            	$comp_columns_conq = DB::table('complex_detail_default')
				->select('complex_detail_default.type','complex_detail_default.ref1','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows',
            		'complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('com_id'))
            	->where('complex_detail_default.type','=',6)
            	->get();
            ...
            $check_rows = array();
            $check_rows = DB::table('asset_complex_detail')
				->select('*')
            	->where('asset_complex_detail.active','=',1)
            	->where('asset_complex_detail.complex_id','=',Input::get('com_id'))
            	->where('asset_complex_detail.node','=',Input::get('node_id'))
            	->where('asset_complex_detail.type','=',4)
            	->get();

            if (empty($check_rows)) {
            	$comp_rows = array();
            	$comp_rows = DB::table('complex_detail_default')
				->select('complex_detail_default.type','complex_detail_default.ref1','complex_detail_default.id','complex_detail_default.complex_id',
            		'complex_detail_default.columns','complex_detail_default.rows',
            		'complex_detail_default.description')
            	->where('complex_detail_default.active','=',1)
            	->where('complex_detail_default.complex_id','=',Input::get('com_id'))
            	->where('complex_detail_default.type','=',4)
            	->get();
	...
	public function getBasictable(){
		...
		return View::make('asset_register.table.table_basic')
			->with('basic',$basic);
	}
	...
	public function getFormdetail(){
		...
		foreach ($detect as $dec) {
			$detection[$dec->rows] = $dec->rows;
		}

		return View::make('asset_register.form.form_fmeca')
			->with('id',Input::get('id'))
			->with('node_id',Input::get('node_id'))
			->with(compact('severity'))
			->with(compact('occorrence'))
			->with(compact('detection'))
			->with('basic_failure',$basic_failure);
	}

	public function postAddfmeca(){

		$rs = DB::table('asset_basic_failure')
            	->where('id',Input::get('id'))
            ...
            [
    			...
    			'ref7'=> Input::get('ref7'),
    			'ref8'=> Input::get('ref8'),
    			'failure_effect_remark'=> Input::get('failure_effect_remark'),
    			'worst_case'=>Input::get('worst_case'),
    			'updated_by'=> Auth::user()->id,
    		]
        );
        ...
        $res = DB::table('asset_registers')
            	->where('id',Input::get('node'))
            	->update(
            [
    			'rpn'=> $max_basic_failure->rpn,
    			'severity'=> $max_basic_failure->severity,
    			'occur'=> $max_basic_failure->occur,
    			'detect'=> $max_basic_failure->detect,
    			'updated_by'=> Auth::user()->id,
    		]
        );
	...
	public function failurecolourcalculate($id=null,$asset_basic_failure_id=null){
		...
		if ((!empty($asset->severity))&&(!empty($asset->occur))&&(!empty($asset->detect))) {
			...
			DB::table('asset_basic_failure')
            	->where('id',$asset_basic_failure_id)
            	->where('node',$id)
            	->update(
            	[
            		'color'=> $step2->description,
    				'failure_effect'=> $step2->ref1,
    				'updated_by'=> Auth::user()->id,
    			]
        	);
		}
